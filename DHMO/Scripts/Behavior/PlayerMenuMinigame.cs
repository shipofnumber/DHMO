...
public abstract class PlayerMenuMinigame : Minigame
{
    ...
    public PlayerMenuMinigame(nint ptr) : base(ptr) { }
    public PlayerMenuMinigame() : base(ClassInjector.DerivedConstructorPointer<PlayerMenuMinigame>()) => ClassInjector.DerivedConstructorBody(this);

    public static TMenu Create<TMenu>() where TMenu : PlayerMenuMinigame
    {
    ...
    [HideFromIl2Cpp]
    public void Begin(out List<GamePlayer> players)
    {
        this.BeginInternal(null!);

        players = [.. GamePlayer.AllPlayers.OrderByDescending(p => p.IsAlive).ThenBy(p => p.PlayerId)];
